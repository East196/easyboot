...
import com.github.east196.core.vo.QueryCondition;
import com.github.east196.ezsb.core.CommonConstant;
import com.github.east196.ezsb.shiro.DataAutorUtils;
import com.github.east196.ezsb.shiro.JwtUtil;
import com.github.east196.ezsb.shiro.SysPermissionDataRule;
...
public class QueryGenerator {
    ...
    public static void installMplus(QueryWrapper<?> queryWrapper, Object searchObj, Map<String, String[]> parameterMap) {
		...
		*/
        ...
        for (String c : ruleMap.keySet()) {
            if (Glue.isNotEmpty(c) && c.startsWith(SQL_RULES_COLUMN)) {
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            try {
                if (judgedIsUselessField(name)){// || !BeanUtil.isReadable(searchObj, name)) {
                    continue;
                }
                ...
                Object value = BeanUtil.getProperty(searchObj, name);
                if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
                    String multiLikeval = value.toString().replace(",,", COMMA);
                    String[] vals = multiLikeval.substring(1, multiLikeval.length()).split(COMMA);
                    final String field = Glue.camelToUnderline(name);
                    if (vals.length > 1) {
                        queryWrapper.and(j -> {
    ...
    public static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap) {
        ...
        if (Glue.isNotEmpty(column) && Glue.isNotEmpty(order)) {
            //字典字段，去掉字典翻译文本后缀
            ...
            if (order.toUpperCase().indexOf(ORDER_TYPE_ASC) >= 0) {
                queryWrapper.orderByAsc(Glue.camelToUnderline(column));
            } else {
                queryWrapper.orderByDesc(Glue.camelToUnderline(column));
            }
        }
    }
    ...
    public static void doSuperQuery(QueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap) {
        if (parameterMap != null && parameterMap.containsKey(SUPER_QUERY_PARAMS)) {
            ...
            List<QueryCondition> conditions = JSONUtil.toList(JSONUtil.parseArray(superQueryParams), QueryCondition.class);
            log.info("---高级查询参数-->" + conditions.toString());

            for (QueryCondition rule : conditions) {
                if (Glue.isNotEmpty(rule.getField()) && Glue.isNotEmpty(rule.getRule()) && Glue.isNotEmpty(rule.getVal())) {
    ...
    private static void addEasyQuery(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value) {
        ...
        name = Glue.camelToUnderline(name);
    ...
    public static String getSingleQueryConditionSql(String field, String alias, Object value, boolean isString) {
        ...
        field = alias + Glue.camelToUnderline(field);
